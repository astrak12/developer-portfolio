...
// Data mentah penilaian masing-masing pengepul terhadap kriteria
const MOCK_NILAI: NilaiEvaluasi[] = [
    { pengepulId: 'p1', nilai: { k1: 80, k2: 5, k3: 100, k4: 80 } }, // Jarak 5km
    { pengepulId: 'p2', nilai: { k1: 90, k2: 15, k3: 80, k4: 70 } }, // Jarak 15km
    { pengepulId: 'p3', nilai: { k1: 70, k2: 2, k3: 150, k4: 90 } }, // Jarak 2km
];
...
export const SpkLab: React.FC = () => {
    ...
    const hasilSAW = useMemo(() => {
        // 1. Cari nilai Max (untuk Benefit) dan Min (untuk Cost) dari setiap kriteria
        const maxMinPerKriteria: Record<string, { max: number, min: number }> = {};
        kriteria.forEach(k => {
            const semuaNilai = MOCK_NILAI.map(n => n.nilai[k.id]);
        ...
        // 2. Normalisasi Matriks & 3. Hitung Nilai Akhir
        const hasil = pengepul.map(p => {
            ...
            kriteria.forEach(k => {
                ...
                if (k.atribut === 'benefit') {
                    nilaiNormalisasi = nilaiAsli / maxMinPerKriteria[k.id].max;
                } else {
                    nilaiNormalisasi = maxMinPerKriteria[k.id].min / nilaiAsli;
                }

                rincianNormalisasi[k.id] = nilaiNormalisasi;
                // Bobot dibagi 100 agar menjadi persentase desimal (misal 30 -> 0.3)
        ...
        // 4. Urutkan berdasarkan total skor tertinggi (Ranking)
    ...
    return (
        <div className="bg-white dark:bg-[#0B1021] rounded-2xl shadow-sm border border-slate-200 dark:border-space-starlight/20 overflow-hidden transition-colors">
            ...
            <div className="p-6 sm:p-8 min-h-[400px]">
                ...
                {activeTab === 'pengepul' && (
                    <div>
                        <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">Alternatif Pengepul</h3>
                        <ul className="space-y-2">
                            {pengepul.map(p => (
                ...
                {activeTab === 'perhitungan' && (
                    <div className="space-y-6">
                        <div>
                            ...
                            <p className="text-sm text-slate-500 mb-4">Tabel di bawah menampilkan hasil perhitungan preferensi (V) yang telah diurutkan dari nilai tertinggi hingga terendah.</p>
                                ...
                                <table className="w-full text-left text-sm">
                                    <thead className="bg-slate-100 dark:bg-[#060913] text-slate-600 dark:text-slate-400">
                                        <tr>
                                            ...
                                            <th className="p-3">Nama Pengepul</th>
